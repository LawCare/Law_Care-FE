...
const Chatting = ({ onExplainWordClick }) => {
  const highlightText = (text) => {
    const parts = text.split(/(퇴직금)/g);
    return parts.map((part, index) =>
      part === '퇴직금' ? (
        <span key={index} className="text-blue-500 cursor-pointer underline" onClick={onExplainWordClick}>
          {part}
        </span>
      ) : (
        part
        ...
        <div className="flex items-start ">
          ...
          <div className="flex flex-col">
            <div className="bg-white rounded-lg p-4 shadow-sm max-w-xs">
              <p>{highlightText('안녕하세요!')}</p>
              <p>{highlightText('你好!')}</p>
            </div>
        ...
        <div className="items-start justify-end ">
          <div className="flex flex-col items-end">
            <div className="bg-blue-100 rounded-md p-2 shadow-sm max-w-xs">
              <p>{highlightText('你好，我没有看到我退休金，受到了不公平的待遇，需要帮助。')}</p>
              <p>{highlightText('안녕하세요. 퇴직금을 못 받고 부당한 대우를 받고 있어요. 도움이 필요해요.')}</p>
            </div>
            <span className="text-xs text-gray-400 mt-1">읽음 1분 전</span>
          </div>
        </div>
        ...
        <div className="flex items-start">
          ...
          <div className="flex flex-col">
            <div className="bg-white rounded-lg p-4 shadow-sm max-w-xs">
              <p>
                {highlightText(
                  '퇴직금과 미지급과 부당 대우에 대한 상황을 자세히 알려주시면 법적 대응 방안을 함께 마련하겠습니다.'
                )}
              </p>
              <p>{highlightText('如果详细告知未支付退休金和不当待遇的情况,将共同制定法律应对方案。')}</p>
            </div>
